...
vignette: >
  ...
  %\VignetteDepends{mirtarbase,BiocStyle}
...
start with *BCL2*. For this we define a `GenenameFilter` specifying
`condition`"like"=.
...
*BCL2* gene family, thus we retrieve next all MTIs between miRNAs of the miRNA
...
grouped by any attribute from the database. It is possible to group the results
...
performance improvement this also enables to select only specific attributes
from the database. Note however that by default the method returns all
attributes from the database which results in a `data.frame` with one
...
## attributes and specify to return the results as a data.frame rather than a list
## of MTI objects.
